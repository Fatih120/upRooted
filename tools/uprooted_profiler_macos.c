/*
 * Uprooted .NET CoreCLR Profiler - IL Injection (macOS port)
 *
 * macOS port of uprooted_profiler_linux.c. Uses POSIX APIs and macOS-specific
 * APIs (e.g. _NSGetExecutablePath) instead of Linux-specific ones.
 ...
 *
 * Strategy (identical to Windows and Linux):
 ...
 *
 * Build: clang -shared -fPIC -O2 -o libuprooted_profiler.dylib tools/uprooted_profiler_macos.c
 */
...
/* CoreCLR uses these types on all platforms. On macOS, we define them
 ...
 * Since we can't use L"..." (that's wchar_t, which is 32-bit on macOS),
...
static void InitPaths(void) {
    ...
    if (!home) home = "/tmp";
    snprintf(g_hookDllPath, PATH_MAX, "%s/Library/Application Support/uprooted/UprootedHook.dll", home);
    snprintf(g_logFilePath, PATH_MAX, "%s/Library/Application Support/uprooted/profiler.log", home);
...
typedef struct {
    uint32_t       Data1;    /* NOT unsigned long — long is 8 bytes on macOS x64 */
    uint16_t       Data2;
    uint16_t       Data3;
...
static HRESULT Prof_Initialize(UprootedProfiler* self, void* pICorProfilerInfoUnk) {
    PLog("=== Uprooted Profiler Initialize (macOS) ===");
    ...
    /* Process guard: only run in Root */
    {
        ...
        if (_NSGetExecutablePath(exePath, &bufSize) != 0) {
            PLog("Could not get executable path via _NSGetExecutablePath, detaching");
            return 0x80004005; /* E_FAIL */
        }
        /* _NSGetExecutablePath may return a relative path; resolve it */
        char resolvedPath[PATH_MAX];
        if (realpath(exePath, resolvedPath) != NULL) {
            strncpy(exePath, resolvedPath, sizeof(exePath) - 1);
        ...
        if (strcmp(exeName, "Root") != 0 && strcasecmp(exeName, "root") != 0) {
            /* Not an exact match — check if the exe name starts with "Root"
             * (e.g. Root.app bundle executable). macOS does not have AppImage,
             * but the app bundle may use a different binary name. */
            int isRoot = 0;

            if (strncmp(exeName, "Root", 4) == 0) {
                isRoot = 1;
            }
            ...
            PLogFmt("Root variant detected (exe='%s'), continuing", exeName);
        }
    }
...
/* ---- Exported functions ---- */
/* On macOS, CoreCLR loads the profiler via dlopen() and calls
...
/* No DllMain equivalent needed on macOS.
 * Initialization is lazy (triggered by DllGetClassObject -> Prof_Initialize).
 * The .dylib is loaded via dlopen() and unloaded via dlclose(). */
