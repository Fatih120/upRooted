...
- **Windows:** `installer/src-tauri/target/release/uprooted.exe`
  (renamed to `Uprooted-0.4.2-Setup.exe` for distribution)
- **Linux:** `installer/src-tauri/target/release/uprooted`
  (renamed to `Uprooted-0.4.2-linux-amd64` for distribution)
...
| ------------------------------------------- | -------- | ---------- | ------------------------------------ |
| `package.json` (`version`)                  | `0.4.2` | semver     | TypeScript build (`__UPROOTED_VERSION__`) |
| `installer/src-tauri/Cargo.toml` (`version`)| `0.4.2` | semver     | Rust crate version, CI rename        |
| `hook/UprootedSettings.cs` (`Version`)      | `0.4.2` | string     | Default version in settings INI file |
| `scripts/install-hook.ps1` (settings write) | `0.4.2` | string     | Written to `uprooted-settings.ini`   |
...
2. **C# hook:** `UprootedSettings.cs` has a hardcoded default
   (`Version = "0.4.2"`) that gets written to `uprooted-settings.ini`.
...
*Build pipeline guide for Uprooted v0.4.2. Last updated 2026-02-19.*
