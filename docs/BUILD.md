...
| ------------------------------------------- | -------- | ---------- | ------------------------------------ |
| `package.json` (`version`)                  | `0.1.95` | semver     | TypeScript build (`__UPROOTED_VERSION__`) |
| `installer/src-tauri/Cargo.toml` (`version`)| `0.1.95` | semver     | Rust crate version, Tauri build      |
| `installer/src-tauri/tauri.conf.json` (`version`) | `0.1.95` | semver | Installer product version, CI rename |
| `installer/package.json` (`version`)        | `0.1.95` | semver     | Installer frontend package           |
| `hook/UprootedSettings.cs` (`Version`)      | `0.1.95` | string     | Default version in settings INI file |
| `scripts/install-hook.ps1` (settings write) | `0.1.95` | string     | Written to `uprooted-settings.ini`   |
...
2. **C# hook:** `UprootedSettings.cs` has a hardcoded default
   (`Version = "0.1.95"`) that gets written to `uprooted-settings.ini`.
