...
|-- package.json                      # pnpm workspace root (v0.3.6-rc).
...
| `AutoUpdater` | `hook/AutoUpdater.cs` | In-process auto-updater. Checks GitHub releases API (stable or dev channel), downloads encrypted `.uprpkg` package, decrypts and unpacks 6 update files to staging, verifies, then overwrites in-place. Developer channel uses XOR-obfuscated PAT for authenticated API access. HTTP via reflection (trimming-safe). |
...
*Architecture reference for Uprooted v0.3.6-rc. Last updated 2026-02-18.*
