...
- **NEVER guess at layout/styling values without consulting ILSpy dumps first**: When debugging layout issues (padding, width, alignment, grid structure), read the actual decompiled UI source code to understand the structure before applying fixes. Don't attempt multiple padding value guesses or trial-and-error adjustments: instead, read the relevant .xaml/.cs files from `research/ilspy-dumps/` (e.g., `HomeView.cs`, `CommunityTabView.cs`) to understand grid columns, row definitions, alignment properties, and measured bounds. Layout bugs typically stem from misunderstanding the actual control hierarchy or property bindings, not from pixel-perfect adjustments.
