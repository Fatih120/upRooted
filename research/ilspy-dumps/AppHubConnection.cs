...
public sealed class AppHubConnection : IAsyncDisposable
{
	...
	private readonly IAppHubConnectionHandler _connectionHandler;
	...
	private readonly ILogger<AppHubConnection> _logger;
	...
	public AppHubConnection(Uri P_0, HttpClient P_1, IAppHubConnectionHandler P_2, ILoggerFactory P_3, Action P_4, CancellationToken P_5)
	{
		...
		_connected = P_4;
		_logger = _loggerFactory.CreateLogger<AppHubConnection>();
	...
	private async Task<bool> RunAsync()
	{
		...
		{
			LogConnecting(Url);
			await _rootWebSocketClient.ConnectAsync(Url, _httpClient, _softShutdownCancellationTokenSource.Token).ConfigureAwait(continueOnCapturedContext: false);
	...
	public async ValueTask SendMessageAsync(AppRpcMessageToHost P_0, CancellationToken P_1)
	{
		if (_disposed)
		{
			return;
		...
		_logger.LogDebug("Sequence Number {SequenceNumber}", P_0.SequenceNumber);
	...
	[LoggerMessage(Level = LogLevel.Error, Message = "AppHubConnection failed.")]
	[GeneratedCode("Microsoft.Gen.Logging", "10.3.0.0")]
	private void LogHubConnectionFailed(Exception P_0)
	{
		LoggerMessageState threadLocalState = LoggerMessageHelper.ThreadLocalState;
		threadLocalState.ReserveTagSpace(1);
		threadLocalState.TagArray[0] = new KeyValuePair<string, object>("{OriginalFormat}", "AppHubConnection failed.");
		_logger.Log(LogLevel.Error, new EventId(39464955, "LogHubConnectionFailed"), threadLocalState, P_0, [GeneratedCode("Microsoft.Gen.Logging", "10.3.0.0")] (LoggerMessageState s, Exception? _) => "AppHubConnection failed.");
		threadLocalState.Clear();
	}
	...
	[LoggerMessage(Level = LogLevel.Error, Message = "ReceivePacketsAsync {Id} failed.")]
	[GeneratedCode("Microsoft.Gen.Logging", "10.3.0.0")]
	private void LogReceiveFailed(Exception P_0, Guid P_1)
	{
		...
		_logger.Log(LogLevel.Error, new EventId(345761385, "LogReceiveFailed"), threadLocalState, P_0, [GeneratedCode("Microsoft.Gen.Logging", "10.3.0.0")] (LoggerMessageState s, Exception? _) =>
		{
	...
	[LoggerMessage(Level = LogLevel.Error, Message = "PacketWriterAsync {Id} failed.")]
	[GeneratedCode("Microsoft.Gen.Logging", "10.3.0.0")]
	private void LogPacketWriterFailed(Exception P_0, Guid P_1)
	{
		...
		threadLocalState.TagArray[0] = new KeyValuePair<string, object>("Id", P_1);
		_logger.Log(LogLevel.Error, new EventId(1931815795, "LogPacketWriterFailed"), threadLocalState, P_0, [GeneratedCode("Microsoft.Gen.Logging", "10.3.0.0")] (LoggerMessageState s, Exception? _) =>
		{
			object value = s.TagArray[0].Value;
			IFormatProvider invariantCulture = CultureInfo.InvariantCulture;
